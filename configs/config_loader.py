...
class ConfigLoader:
    ...
    def __init__(self, config_path=None):
        ...
        if config_path and my_file.is_file():
            ...
            self.config["SLACK_CLIENT_TOKEN"] = os.getenv("SLACK_CLIENT_TOKEN", "ERROR")
            ...
            self.config["CHECK_MK_PASSWORD"] = os.getenv("CHECK_MK_PASSWORD", "ERROR")
            self.config["FUNCTION_RETURN_FORMAT"] = []

    def __load_config_file(self, config_path):
        with open(config_path, "r") as ymlfile:
            self.config = yaml.load(ymlfile, Loader=yaml.SafeLoader)
